...
<SCRIPT>
	...
	function changeCategory(strForm)
	{
		...
		document.frmQYearReportFormRefresh.elements(0).value  = strForm.elements(0).value;		
		if (strForm.elements(1).disabled==true)
		{
			document.frmQYearReportFormRefresh.elements(1).value = 0;		
		}
		else
		{
			document.frmQYearReportFormRefresh.elements(1).value = parseInt(strForm.elements(1).value);
		}
			
		if (strForm.elements(2).checked==true)
		{
			document.frmQYearReportFormRefresh.elements(2).value  = strForm.elements(2).value;
		}
		else
		{
			document.frmQYearReportFormRefresh.elements(2).value  = 0; 
		}
		
		if (strForm.elements(3).checked==true)
		{
			document.frmQYearReportFormRefresh.elements(3).value  = strForm.elements(3).value;
		}
		else
		{
			document.frmQYearReportFormRefresh.elements(3).value  = 0; 
		}	
		
		if (strForm.elements(4).checked==true)
		{
			document.frmQYearReportFormRefresh.elements(4).value  = strForm.elements(4).value;
		}
		else
		{
			document.frmQYearReportFormRefresh.elements(4).value  = 0; 
		}
		
		if (strForm.elements(5).checked==true)
		{
			document.frmQYearReportFormRefresh.elements(5).value  = strForm.elements(5).value;
		}
		else
		{
			document.frmQYearReportFormRefresh.elements(5).value  = 0; 
		}
		
		document.frmQYearReportFormRefresh.elements(6).value  = strForm.elements(6).value;		
		document.frmQYearReportFormRefresh.elements(7).value  = strForm.elements(7).value;		
		document.frmQYearReportFormRefresh.elements(8).value  = strForm.elements(8).value;		
		document.frmQYearReportFormRefresh.elements(9).value  = strForm.elements(9).value;		
		document.frmQYearReportFormRefresh.elements(10).value  = strForm.elements(10).value;		
		
		document.frmQYearReportFormRefresh.submit();
	} 
...
//		nToDay       = parseInt(strForm.ToDay.value)
		...
		if (strForm.LastQYear.checked==false)
		{
...
<!---------------  The Top Navigation of IDSC Memory  ----------------------->
	...
	<TABLE width="100%" border=0 cellspacing=0 cellpadding=0 align="CENTER" height="100%" bgcolor="#336699">
		...
		<TD width="52%" valign="TOP" align="CENTER">
				...
				<TR>
					<TD width="100%" align="CENTER" valign="TOP" bgcolor="#ffffff">
							...
							<TABLE width="90%" align="CENTER" border=0 cellspacing=0 cellpadding=0 bgcolor="WHITE">
								<TR>
									...
									<TD align="CENTER" width="80%" valign="MIDDLE" bgcolor="#b0c4de">
										...
										بحث تقارير الإنجازات الربع سنوية
										</B></FONT>
									</TD>
								...
								<TR>
									<TD align="LEFT" colspan=3>
										<FORM name="qyear_report_search" action="qyear_report_search_result.asp" method="POST">
						...
						-->																
										...
										-->				</TR>
											...
											<TR>
												<TD width="70%" align="RIGHT" valign="TOP" bgcolor="#e6e6ea">
													...
													<B>تقرير الإنجاز الربع سنوى الحالى</B>
													</FONT>
													<INPUT type="CHECKBOX" name="LastQYear" value="1" onClick="enableComboBox(this);">
												</TD>
												<TD width="30%" align="RIGHT" valign="MIDDLE" bgcolor="#b0c4de">
							...
							<FORM name="frmQYearReportFormRefresh" action="qyear_report_search.asp" method="POST">
			...
			-->					<INPUT type="HIDDEN" name="LastQYear"   value=0>
