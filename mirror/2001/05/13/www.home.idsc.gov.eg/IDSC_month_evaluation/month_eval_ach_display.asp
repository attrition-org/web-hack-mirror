...
<SCRIPT>
	...
	function validateForm(strForm)
	{
		...
		{	
			alert("من فضلك : إختر أحد القطاعات او البرامج");
			bRetVal = false;
		}
		
		bTempVal = checkSelection(parseInt(strForm.EvalEvaluation.value),0);
		if (bTempVal==false && bRetVal==true)   
		{	
			alert("من فضلك : إختر التقييم الشهرى");
			bRetVal = false;
		}
		
		bTempVal = checkSelection(parseInt(strForm.EvalYear.value),0);
		if (bTempVal==false && bRetVal==true)   
		{	
		...
		bTempVal = checkSelection(parseInt(strForm.EvalMonth.value),0);
		if (bTempVal==false && bRetVal==true)   
		{	
			alert("من فضلك : إختر الشهر");
			bRetVal = false;
		}
		
		bTempVal  = checkIsEmpty(strForm.EvalWeight.value);
		bTempVal1 = checkIsNumber(strForm.EvalWeight.value);
		
		if ((bTempVal==false || bTempVal1==false) && bRetVal==true)   
		{	
			alert("من فضلك : أدخل الترتيب");
			bRetVal = false;
		}
			...
			<TABLE width="100%" align="CENTER" border=0 bgcolor="WHITE" cellpadding=3>
				<TR>
					<TD width="100%" align="CENTER">
						<BR>
						<IMG src="../images/week_report_entry/month_subtitle.jpg">
					</TD>
				</TR>
				
				<TR>
					<TD width="100%" align="CENTER">
						<FONT color="#ff6347" face="ARIAL" style="FONT-SIZE:15PT"><B>
						محور القطاعات
						</B></FONT><BR><BR>
